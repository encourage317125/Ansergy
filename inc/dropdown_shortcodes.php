<?php
//include 'php-browser-detection/php-browser-detection.php';

/*echo trailingslashit(dirname(__FILE__)); exit;*/
add_action( 'wp_enqueue_scripts', 'custom_scripts' );
function custom_scripts()
{
	wp_enqueue_style( 'daterangepicker', get_template_directory_uri() . '/inc/datepicker/daterangepicker.css', array(), '' );
	wp_enqueue_script( 'moment', get_template_directory_uri() . '/inc/datepicker/moment.min.js', array(), ''  );
	wp_enqueue_script( 'daterangepicker', get_template_directory_uri() . '/inc/datepicker/daterangepicker.js', array(), ''  );
}

/*
* Shortcode for dropdown listed below
*/
...
function form_func($atts, $content = null)
{
	...
	$output = '';
	$output .= '<div class="row col-md-12 drops_from">';
	$output .= '<div class="alert alert-error" id="alert"><strong>Oh snap!</strong></div>';
	$output .= '<form action="" method="get" class="col-md-12 row">';

	$output .= do_shortcode( $content );
	if($atts["date_range"] == "true")
	{
		// Start Date
		$output .= '<div id="two-inputs" class="">';
		$output .= '<div class="col-md-4 col-xs-6">';
		$output .= '<div class="form-group">';
    	$output .= '<label>Start Date</label>';
		$output .= '<input type="text" id="start_date" class="form-control" value="'.$_GET["start_date"].'" name="start_date" required>';
		$output .= '</div>';
		$output .= '</label>';
		$output .= '</div>';

		// End Date
		$output .= '<div class="col-md-4 col-xs-6 end_date">';
		$output .= '<div class="form-group">';
    	$output .= '<label>End Date</label>';
		$output .= '<input type="text" class="form-control" value="'.$_GET["end_date"].'" name="end_date" id="end_date" required>';
		$output .= '</div>';
		$output .= '</label>';
		$output .= '</div>';
